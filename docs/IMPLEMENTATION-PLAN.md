...
------------------------------------------------------------------
16 ADRs (docs only)                    Full working application
No src/ directory                      NX monorepo (apps/api, apps/web)
No package.json                        NestJS + React + all deps
No tests                               >80% domain test coverage
No Docker                              Docker Compose + K8s manifests
No CI/CD                               GitHub Actions pipeline
Branch: ddd-approach                   Merged to main via PR
```
...
| # | Task | Agent | Files Created | Success Criteria |
|---|------|-------|---------------|------------------|
| 0.1 | Initialize NX workspace | `system-architect` | `package.json`, `nx.json`, `tsconfig.base.json`, `.eslintrc.json`, `.prettierrc` | `npm install` succeeds |
| 0.2 | Create NestJS API app | `system-architect` | `apps/api/src/main.ts`, `apps/api/src/app.module.ts`, `apps/api/project.json`, `apps/api/tsconfig.app.json`, `apps/api/tsconfig.spec.json` | `nx build api` succeeds |
| 0.3 | Create React web app | `system-architect` | `apps/web/src/main.tsx`, `apps/web/src/App.tsx`, `apps/web/index.html`, `apps/web/vite.config.ts`, `apps/web/project.json`, `apps/web/tsconfig.app.json` | `nx build web` succeeds |
| 0.4 | Create library scaffold | `system-architect` | `libs/shared/types/src/index.ts`, `libs/shared/utils/src/index.ts`, `libs/shared/constants/src/index.ts` + project.json for each | `nx build shared-types` succeeds |
| 0.5 | Configure TypeScript paths | `system-architect` | Updates to `tsconfig.base.json` | `@csn/shared-types` import resolves |
| 0.6 | Add root scripts | `system-architect` | Updates to root `package.json` | `npm run build`, `npm run test`, `npm run lint` work |
| 0.7 | Configure ESLint + Prettier | `system-architect` | `.eslintrc.json` (root), per-app `.eslintrc.json` | `nx lint api` and `nx lint web` pass |
| 0.8 | Add `.gitignore`, `.nvmrc`, `.editorconfig` | `system-architect` | `.gitignore`, `.nvmrc`, `.editorconfig` | Files exist |

### Quality Gate

- [ ] `npm install` completes without errors
- [ ] `nx build api` produces dist output
- [ ] `nx build web` produces dist output
- [ ] `nx lint api` and `nx lint web` pass
- [ ] `nx graph` shows correct dependency tree (web, api, libs)
...
| # | Task | Files Created |
|---|------|---------------|
| 1.1 | TypeORM configuration module | `libs/infrastructure/database/src/database.module.ts`, `libs/infrastructure/database/src/database.config.ts`, `libs/infrastructure/database/src/typeorm.config.ts` |
| 1.2 | Redis client module | `libs/infrastructure/cache/src/redis.module.ts`, `libs/infrastructure/cache/src/redis.config.ts` |
| 1.3 | Three-tier cache service (ADR-010) | `libs/infrastructure/cache/src/three-tier-cache.service.ts`, `libs/infrastructure/cache/src/stampede-protection.ts`, `libs/infrastructure/cache/src/cache.interface.ts` |

#### Group B: Event Bus + Messaging (Agent: `coder-2`)

| # | Task | Files Created |
|---|------|---------------|
| 1.4 | In-process event dispatcher | `libs/infrastructure/messaging/src/in-process-event-dispatcher.ts`, `libs/infrastructure/messaging/src/event-dispatcher.interface.ts` |
| 1.5 | Bull Queue integration publisher (ADR-009) | `libs/infrastructure/messaging/src/bull-queue-event-publisher.ts`, `libs/infrastructure/messaging/src/integration-event-publisher.interface.ts`, `libs/infrastructure/messaging/src/bull.config.ts` |
| 1.6 | Dead letter queue handler (ADR-009) | `libs/infrastructure/messaging/src/dead-letter-queue.ts` |
| 1.7 | Idempotency store | `libs/infrastructure/messaging/src/idempotency-store.ts` |
| 1.8 | Socket.IO gateway scaffold | `libs/infrastructure/messaging/src/socket-io-event-emitter.ts`, `libs/infrastructure/messaging/src/socket-io.gateway.ts` |

#### Group C: DDD Shared Kernel (Agent: `coder-3`)

| # | Task | Files Created |
|---|------|---------------|
| 1.9 | AggregateRoot base class | `libs/domain/shared/src/aggregate-root.ts` |
| 1.10 | Entity base class | `libs/domain/shared/src/entity.ts` |
| 1.11 | ValueObject base class | `libs/domain/shared/src/value-object.ts` |
| 1.12 | DomainEvent base class (ADR-009) | `libs/domain/shared/src/domain-event.ts` |
| 1.13 | Repository interface | `libs/domain/shared/src/repository.interface.ts` |
| 1.14 | Shared value objects: UserId, Email, Timestamp | `libs/domain/shared/src/value-objects/user-id.ts`, `email.ts`, `timestamp.ts` |
| 1.15 | Domain constants (ADR-008) | `libs/domain/shared/src/constants/domain-constants.ts` |
| 1.16 | Pagination types | `libs/domain/shared/src/types/pagination.ts` |
| 1.17 | Domain error base classes | `libs/domain/shared/src/errors/domain-error.ts` |

#### Group D: Auth Module (Agent: `coder-4`)

| # | Task | Files Created |
|---|------|---------------|
| 1.18 | JWT service (ADR-004) | `libs/infrastructure/auth/src/jwt.service.ts`, `libs/infrastructure/auth/src/jwt.config.ts` |
| 1.19 | Auth guard (NestJS) | `libs/infrastructure/auth/src/auth.guard.ts`, `libs/infrastructure/auth/src/current-user.decorator.ts` |
| 1.20 | Token blacklist (Redis) | `libs/infrastructure/auth/src/token-blacklist.service.ts` |
| 1.21 | Key rotation support (ADR-004) | `libs/infrastructure/auth/src/key-rotation.service.ts` |

#### Testing (Agent: `tester-1`, after Groups A-D complete)

| # | Task | Files Created |
|---|------|---------------|
| 1.22 | Unit tests for cache service | `libs/infrastructure/cache/src/__tests__/three-tier-cache.service.spec.ts` |
| 1.23 | Unit tests for event dispatcher | `libs/infrastructure/messaging/src/__tests__/in-process-event-dispatcher.spec.ts` |
| 1.24 | Unit tests for shared kernel | `libs/domain/shared/src/__tests__/aggregate-root.spec.ts`, `value-object.spec.ts`, `domain-event.spec.ts` |
| 1.25 | Unit tests for shared value objects | `libs/domain/shared/src/__tests__/user-id.spec.ts`, `email.spec.ts`, `timestamp.spec.ts` |

### Quality Gate

- [ ] All shared kernel classes compile with zero external dependencies
- [ ] `DomainEvent`, `AggregateRoot`, `ValueObject` have no `@nestjs` or `typeorm` imports
- [ ] Cache service implements 3-tier lookup (memory -> Redis -> null)
- [ ] Event dispatcher dispatches events to registered handlers
- [ ] JWT service generates and validates tokens
- [ ] All unit tests pass (`nx test shared-domain`, `nx test infrastructure-cache`, etc.)
- [ ] Architecture fitness test: `grep -r "from '@nestjs" libs/domain/` returns 0 matches
